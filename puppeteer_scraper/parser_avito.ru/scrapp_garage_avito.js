...
const load_cards = async (page, index, url) => {
    ...
    if (handle) {
        ...
        try {
            ...
            properties.id_avito = await page.$eval("span[data-marker='item-view/item-id']", el => el.textContent);
            properties.date_publication = getDateFromString(await page.$eval("span[data-marker='item-view/item-date']", el => el.textContent));
...
(async function get_garage() {
    ...
    const excel_path = "/excel_data/data.xlsx";
    ...
    if (fs.existsSync(data_path)) {
        ...
        console.log("\n\tВсего ссылок:\t", all, "\n\tЗагружено:\t", all_areLoaded, "\n\tОсталось:\t", all_areNotLoaded, "\n");
        console.log("Создаю excel файл из json.....")
        await json_to_excel(data_path, "excel_data.xlsx");
        process.exit();
    } else {
