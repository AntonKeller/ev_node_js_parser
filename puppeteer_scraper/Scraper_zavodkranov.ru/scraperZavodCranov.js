...
(async function a() {
    ...
    let weightElemenets = await page.$$eval("div.properties_table > ul > li.gryz_id", links => links.map(link => link.getAttribute("data-toggler")));
    let widthElements = await page.$$eval("div.properties_table > ul > li.prolet_id", links => links.map(link => link.getAttribute("data-toggler")));
